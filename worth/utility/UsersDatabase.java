...
public class UsersDatabase implements Serializable {
    ...
    public static void cancellaProgetto(String projectName) {
        for (String user : utenti.keySet()) {
            if (utenti.get(user).listProjects().contains(projectName)) {
                utenti.get(user).listProjects().remove(projectName);
            }
        }
    }
}
