...
public class ServerTCP implements Runnable {
    ...
    public ServerTCP(ConcurrentHashMap<String, String> user_status, ServerNotification serverN, UpdateHashmap up,
            Database DB, UsersDatabase usersDB, Chats C, int portTCP) {
                        ...
                        if (numBytesR == -1) { // Read non riuscita.
                            ...
                            if (cmd.startsWith("login")) {
                                ...
                                if (usersDB.getUtenti().containsKey(username)) {
                                    if (usersDB.getUser(username).getStatus() == "Offline") {
                                        if (usersDB.getUser(username).getPassword().equals(password)) {
                                            synchronized (usersDB) {
                                                ...
                                                try {
                                                    ...
                                                    if (C.getChats() != null && usersDB.getUtenti() != null) {
                                                        up.updateAtLogin(C.getChats(), usersDB.getUtenti());
                                                    }
                                                } catch (Exception e) {
                                                    e.printStackTrace();
                                                }
                                        ...
                                        risposta = "< Operazione negata: login già effettuato";
                                    }
                                } else {
                                ...
                                Arrays.toString(token);
                                username = token[0];
                                if (usersDB.getUser(username).getStatus() == "Online") {
                                    risposta = comm.command(cmd, DB, usersDB, C, serverN, up);
