                ...
                try {
                    if (key.isAcceptable()) {
                        ...
                        if (numBytesR == -1) { // Read non riuscita.
                            ...
                            } else if (cmd.startsWith("list_projects hash")) {
                                String[] token = cmd.split(" ");
                                Arrays.toString(token);
                                String user = token[2];
                                risposta = comm.command(cmd, usersDB.getUser(user), DB, usersDB, C, serverN, up);
                            } else if (usersDB.getUser(username).getStatus() == "Online") {
                                risposta = comm.command(cmd, usersDB.getUser(username), DB, usersDB, C, serverN, up);
