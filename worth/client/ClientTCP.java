...
public class ClientTCP {
    ...
    public void startConnection() throws IOException {
        ...
        UpdateClientInterface clientUP = null;
        ClientNotificationInterface stub = null;
        UpdateClientInterface stub2 = null;
        ...
        while (true) {
            ...
            if (cmd.startsWith("register")) {
                ...
                if (username.isBlank()) {
                    System.out.println(" ");
                    System.out.println("< Inserire un nome valido. Il nome deve contenere almeno un carattere");
                    System.out.println(" ");
                    System.out.println("< Inserire username");
                    username = scanner.nextLine();
                }
                ...
                while (!connesso) {
                    try {
                        ...
                        clientUP = new UpdateClient(chatList, username);
                        stub2 = (UpdateClientInterface) UnicastRemoteObject.exportObject(clientUP, 0);
                        connesso = true;
                ...
                if (!registrazione) {
                    System.out.println("< Digitare 'register'");
                    username = scanner.nextLine();
                    System.out.println(" ");
                    System.out.println("< Inserire username");
                    username = scanner.nextLine();
                ...
                try {
                    ...
                    while (!open) {
                        try {
                            ...
                            System.out.println(" ");
                            System.out.println(
                                    "Digitare SI per ritentare la connessione al servizio oppure NO per uscire");
                            System.out.println(" ");
                            String digit = scanner.nextLine();
                            if (digit.startsWith("SI")) {
                                try {
                    ...
                    if (s.equals("< Operazione negata: utente non registrato.")
                            || s.equals("< Operazione negata: login già effettuato")) {
                        System.out.println(" ");
                        cmd = scanner.nextLine();
                        System.out.println(" ");
                        server.unregisterForCallback(stub);
                        up.unregisterForCB(stub2);
                        effettuato = false;
                        cmdTrovato = false;
                        logIN = false;
                    } else
        ...
        try {
            ...
            while (true) {
                if (cmd.startsWith("Help")) {
                    ...
                    notSend = true;
                } else if (cmd.equals("send_chat_msg")) {
                    cmdTrovato = true;
                    if (logIN == true) {
                        System.out.println(" ");
                        System.out.println("< Inserire nome progetto.");
                ...
                if (!cmdTrovato) {
                    ...
                    if (s.equals("< login effettuato")) {
                        ...
                        System.out.println(s);
                        System.out.println(" ");
                    } else if (s.contains("< Operazione negata: login già effettuato")) {
                        server.unregisterForCallback(stub);
                        up.registerForCB(stub2);
                    } else if (s.equals("< logout effettuato")) {
                        logIN = false;
            ...
            UnicastRemoteObject.unexportObject(clientUP, true);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
